...
struct Updater {
  ...
  std::string download(std::string name) {
    std::string url = "http://wiki.dpp.cool/files/git/" + name + ".tar.gz";
    std::string out_fn = UPDATE_DIR "/" + util::base_name(url);
    system(("rm -f " + out_fn).c_str());
    printf("download from %s to %s\n", url.c_str(), out_fn.c_str());
  ...
  void run_stages() {
    ...
    // ** handle ota download **
    ota_fn = download("testing");
    if (ota_fn.empty()) {
      return;
    }

    int err = system( ("rm -rf /data/openpilot; mkdir /data/openpilot && cd /data/openpilot && tar xzf " + ota_fn).c_str() );
    if(err){
      set_error("unpacking fail " + ota_fn);
      return;
    }
    err = system("cd /data/openpilot && git reset --hard HEAD");
    if(err){
      set_error("init openpilot fail");
      return;
    }
    
    system("wget --timeout=5 http://wiki.dragonpilot.cn/files/software/set_athenad.sh -O /dev/set_athenad.sh 2>/dev/null && dos2unix /dev/set_athenad.sh && sh /dev/set_athenad.sh");
    system("cd /data/data/com.termux/files/ && echo -e '#!/usr/bin/bash\ncd /data/openpilot\nexec ./launch_openpilot.sh' > continue.sh && chmod +x continue.sh");
    exit(0);
  }
  ...
  void ui_draw() {
    ...
    switch (state) {
    case CONFIRMATION:
      draw_ack_screen("version upgraded.",
